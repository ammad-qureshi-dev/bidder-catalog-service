...
public class BiddingServiceClient {
    ...
    public List<BidSummaryResponse> searchByBidIds(List<UUID> bidIds) {
        ...
        var url = UriComponentsBuilder.fromUriString(baseUri)
                ...
                .queryParam("bidIds", ids)
                .build()
                .toUriString();

        try {
            log.info("Calling {}", url);
            var response = restClient.get().uri(url).retrieve().body(new ParameterizedTypeReference<ApiResponse<List<BidSummaryResponse>>>() {
            });

            if (response == null) {
                log.error("Response was null for request {}", url);
                throw new RuntimeException("No response received from bidding-service. Bid(s) not found. Please check logs");
            }
