...
public class IdentityAndAuthServiceClient {
    ...
    /**
     * Internal endpoint; retrieves the preferred contacts of all appUserIds provided
     * @param appUserIds all requested app user ids
     */
    public List<PreferredContactMethod> internalGetPreferredContactMethods(List<UUID> appUserIds) {
        ...
        var url = UriComponentsBuilder.fromUriString(baseUri)
                .path(endpoints.getPreferredContactMethods())
                .queryParam("appUserIds", ids)
                .build()
                .toUriString();

        try {
            log.info("Calling {}", url);
            var response = restClient.get().uri(url).retrieve().body(new ParameterizedTypeReference<ApiResponse<List<PreferredContactMethod>>>() {
            });
...
//            throw new NoSuchElementException("Item not found");
    ...
    public Optional<AppUserDto> getAppUserById(UUID appUserId) {
        ...
        var url = UriComponentsBuilder.fromUriString(baseUri)
                .path(endpoints.getPreferredContactMethods())
                .pathSegment(appUserId.toString())
                .build()
                .toUriString();

        try {
            log.info("Calling {}", url);
            var response = restClient.get().uri(url).retrieve().body(new ParameterizedTypeReference<ApiResponse<AppUserDto>>() {
            });
